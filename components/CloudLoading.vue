<template>
  <v-row justify="center">
    <v-col
      cols="11"
      class="ma-3"
    >
    <v-card class="card" loading elevation="5">
      <v-card-title class="justify-center">Cloud instance is starting...</v-card-title>
      <v-card-subtitle align="center">Why do you have to wait?</v-card-subtitle>
      <v-card-text align="center">
        We start our server only on demand... and this takes a few seconds before
        you can use our free tools (up to 30 seconds).
        <br>
        This is aligned with an energy sobriety strategy. So be patient
        <v-icon color="primary" size="20">
          mdi-emoticon-excited-outline
        </v-icon>
        <br>
        We are currently trying to reduce this waiting launch time to improve your
        experience with our free tools.
      </v-card-text>
    </v-card>
    </v-col>
  </v-row>
</template>

<script>
export default {
  name: 'CloudLoading',
}
</script>
